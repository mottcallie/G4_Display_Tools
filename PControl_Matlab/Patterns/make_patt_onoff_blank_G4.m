...
emptyImage = zeros(frameN, frameM) * bckGS; %initialize

% background only
for x = 1:frameM
    if mod(x,2) %if not even, dark
        Pats(:,:,x,1) = emptyImage;
    else
        Pats(:,:,x,1) = bckImage;
    end
...
%save lookup table
patlookup.fullname = [num2str(bckGS) 'gs_onoff_background'];
...
pattName = [sprintf('%04d', pattN) '_' 'onoff_background'];
